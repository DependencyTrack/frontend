<template>
  <b-modal id="serviceDetailsModal" size="lg" hide-header-close no-stacking :title="$t('message.service_details')">
    <b-tabs class="body-bg-color" style="border:0;padding:0">
      <b-tab class="body-bg-color" style="border:0;padding:0" active>
        ...
        <b-card>
          <b-input-group-form-input id="service-name-input" input-group-size="mb-3" type="text" v-model="service.name"
                                    lazy="true" required="true" feedback="true" autofocus="false"
                                    :label="$t('message.service_name')" :tooltip="this.$t('message.service_name_desc')"
                                    :feedback-text="$t('message.required_service_name')"
                                    :readonly="this.isNotPermitted(PERMISSIONS.PORTFOLIO_MANAGEMENT)" />
          <b-input-group-form-input id="service-version-input" input-group-size="mb-3" type="text" v-model="service.version"
                                    required="false" :label="$t('message.version')"
                                    :tooltip="this.$t('message.service_version_desc')"
                                    :readonly="this.isNotPermitted(PERMISSIONS.PORTFOLIO_MANAGEMENT)" />
          <b-input-group-form-input id="service-group-input" input-group-size="mb-3" type="text" v-model="service.group"
      ...
      <b-tab>
        <template v-slot:title><i class="fa fa-building-o"></i> {{ $t('message.provider') }}</template>
        <b-card>
          <b-input-group-form-input id="service-provider-name-input" input-group-size="mb-3" type="text" v-model="service.provider.name"
                                    required="false" :label="$t('message.provider_name')"
                                    :tooltip="this.$t('message.service_provider_name_desc')"/>
          <b-form-group id="providerUrlsTable-Fieldset" :label="this.$t('message.urls')" label-for="providerUrlsTable">
            <bootstrap-table
          ...
          <b-form-group id="contactsTable-Fieldset" :label="this.$t('message.contacts')" label-for="contactsTable">
            <bootstrap-table
              id="contactsTable"
              ref="contactsTable"
      ...
      <b-tab>
        <template v-slot:title><i class="fa fa-globe"></i> {{ $t('message.endpoints') }}</template>
        <b-card>
          <bootstrap-table
            ref="endpointTable"
      ...
      <b-tab>
        <template v-slot:title><i class="fa fa-server"></i> {{ $t('message.data') }}</template>
        <b-card>
          <bootstrap-table
            ref="dataTable"
    ...
    <template v-slot:modal-footer="{ cancel }">
      <b-button size="md" variant="outline-danger" @click="deleteService()" v-permission="PERMISSIONS.PORTFOLIO_MANAGEMENT">{{ $t('message.delete') }}</b-button>
      <b-button size="md" variant="secondary" @click="cancel()">{{ $t('message.close') }}</b-button>
      <b-button size="md" variant="primary" @click="updateService()" v-permission="PERMISSIONS.PORTFOLIO_MANAGEMENT">{{ $t('message.update') }}</b-button>
    </template>
  </b-modal>
</template>
...
export default {
  ...
  methods: {
    updateService: function() {
      ...
      this.axios.post(url, this.service).then((response) => {
    ...
    deleteService: function() {
      this.$root.$emit('bv::hide::modal', 'serviceDetailsModal');
      let url = `${this.$api.BASE_URL}/${this.$api.URL_SERVICE}/` + this.service.uuid;
      this.axios.delete(url).then((response) => {
        this.$toastr.s(this.$t('message.service_deleted'));
        this.$router.replace({ path: "/projects/" + this.service.project.uuid });
      }).catch((error) => {
        this.$toastr.w(this.$t('condition.unsuccessful_action'));
      });
    }
