<template>
  <b-modal id="licenseAddLicenseModal" @hide="resetValues()" size="md" hide-header-close no-stacking :title="$t('message.add_license')">
    <b-tabs class="body-bg-color" style="border:0;padding:0">
      <b-tab class="body-bg-color" style="border:0;padding:0" active>
        ...
        <b-card>
          <b-input-group-form-input id="license-name-input" input-group-size="mb-3" type="text" v-model="license.name"
                                    lazy="true" required="true" feedback="true" autofocus="false"
                                    :label="$t('message.license_name')" :tooltip="this.$t('message.license_name_desc')"
                                    :feedback-text="$t('message.required_license_name')" />
          <b-input-group-form-input id="license-id-input" input-group-size="mb-3" type="text" v-model="license.licenseId"
                                    lazy="true" required="true" feedback="true" autofocus="false"
                                    :label="$t('message.license_id')" :tooltip="this.$t('message.license_id_desc')"
      ...
      <b-tab>
        ...
        <b-card>
          <b-form-group
            ...
            <b-form-textarea id="license-comments-description" v-model="license.licenseComments" rows="3" />
          </b-form-group>
          <b-form-group
            id="license-template-form-group"
            :label="this.$t('message.template')"
            label-for="license-template-input">
            <b-form-textarea id="license-template-description" v-model="license.standardLicenseTemplate" rows="3" />
          </b-form-group>
          <b-form-group
            id="license-source-header-form-group"
            :label="this.$t('message.source_header')"
            label-for="license-source-header-input">
            <b-form-textarea id="license-source-header-description" v-model="license.standardLicenseHeader" rows="3" />
          </b-form-group>
          <b-form-group
            id="license-see-also-form-group"
            :label="this.$t('message.see_also')"
            label-for="license-see-also-input">
            <b-form-textarea id="license-see-also-description" v-model="license.seeAlso" rows="3" />
          </b-form-group>
        </b-card>
      </b-tab>
    </b-tabs>
    <template v-slot:modal-footer="{ cancel }">
      ...
      <b-button size="md" variant="primary" @click="addLicense()">{{ $t('message.create') }}</b-button>
    </template>
  </b-modal>
</template>
