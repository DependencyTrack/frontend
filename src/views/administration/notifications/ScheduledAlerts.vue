<template>
  <b-card no-body :header="header">
    <b-card-body>
      <div id="customToolbar">
        <b-button
          ...
          v-b-modal.createScheduledAlertModal
        >
          <span class="fa fa-plus"></span>
          {{ $t('admin.create_scheduled_alert') }}
        </b-button>
      </div>
      <bootstrap-table
        ref="table"
    ...
    <create-scheduled-alert-modal v-on:refreshTable="refreshTable" />
  </b-card>
</template>
...
import CreateScheduledAlertModal from './CreateScheduledAlertModal';
...
export default {
  ...
  mounted() {
    EventBus.$on('admin:scheduledalerts:rowUpdate', (index, row) => {
      this.$refs.table.updateRow({ index: index, row: row });
      this.$refs.table.expandRow(index);
    });
    EventBus.$on('admin:scheduledalerts:rowDeleted', (index, row) => {
      this.refreshTable();
    });
  },
  beforeDestroy() {
    EventBus.$off('admin:scheduledalerts:rowUpdate');
    EventBus.$off('admin:scheduledalerts:rowDeleted');
  },
  data() {
    return {
      columns: [
      ...
      options: {
            ...
            methods: {
              ...
              deleteNotificationRule: function () {
                ...
                this.axios
                  ...
                  .then((response) => {
                    EventBus.$emit('admin:scheduledalerts:rowDeleted', index);
                    this.$toastr.s(this.$t('admin.scheduled_alert_deleted'));
                  })
                  .catch((error) => {
                    this.$toastr.w(this.$t('condition.unsuccessful_action'));
                  });
              ...
              deleteLimiter: function (projectUuid) {
                let url = `${this.$api.BASE_URL}/${this.$api.URL_SCHEDULED_NOTIFICATION_RULE}/${this.uuid}/project/${projectUuid}`;
                this.axios
                  .delete(url)
                  .then((response) => {
                    let p = [];
              ...
              updateProjectSelection: function (selections) {
                ...
                for (let i = 0; i < selections.length; i++) {
                  ...
                  let url = `${this.$api.BASE_URL}/${this.$api.URL_SCHEDULED_NOTIFICATION_RULE}/${this.uuid}/project/${selection.uuid}`;
                  this.axios
                    .post(url)
                    .then((response) => {
              ...
              updateTeamSelection: function (selections) {
                ...
                for (let i = 0; i < selections.length; i++) {
                  let selection = selections[i];
                  let url = `${this.$api.BASE_URL}/${this.$api.URL_SCHEDULED_NOTIFICATION_RULE}/${this.uuid}/team/${selection.uuid}`;
                  this.axios
                    .post(url)
                    .then((response) => {
                      if (this.teams) {
              ...
              removeSelectedTeam: function (teamUuid) {
                let url = `${this.$api.BASE_URL}/${this.$api.URL_SCHEDULED_NOTIFICATION_RULE}/${this.uuid}/team/${teamUuid}`;
                this.axios
                  .delete(url)
                  .then((response) => {
                    let newTeams = [];
        ...
        url: `${this.$api.BASE_URL}/${this.$api.URL_SCHEDULED_NOTIFICATION_RULE}`,
      },
    };
  },
  methods: {
