<template>
  <b-modal id="createAlertModal" size="md" hide-header-close no-stacking :title="$t('admin.create_alert')">
    <b-form-group
      id="fieldset-1"
    ...
    <b-form-group
      ...
      label-for="input-2">
      <b-form-select id="input-2" v-model="scope" :options="availableScopes" required></b-form-select>
    </b-form-group>
    <b-form-group
      id="fieldset-3"
    ...
    <b-form-group
      ...
      label-for="input-4">
      <b-form-select id="input-4" v-model="publisher" :options="availablePublishers" required></b-form-select>
    </b-form-group>
    <template v-slot:modal-footer="{ cancel }">
      <b-button size="md" variant="secondary" @click="cancel()">{{ $t('message.close') }}</b-button>
      <b-button size="md" variant="primary" @click="createAlert()">{{ $t('message.create') }}</b-button>
    </template>
  </b-modal>
</template>
...
<script>
  ...
  export default {
    ...
    created() {
      axios.get(`${this.$api.BASE_URL}/${this.$api.URL_NOTIFICATION_PUBLISHER}`)
        .then((result) => {
            for (let i=0; i<result.data.length; i++) {
              this.availablePublishers.push({value: result.data[i].uuid, text: result.data[i].name});
            }
          }
        );
    },
