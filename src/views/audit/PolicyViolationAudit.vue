    ...
    <b-row>
      <b-col xs="6" sm="4" md="4" lg="2" id="filter-controls">
        <div>
          ...
          <b-form-group id="violation-state-form-group" :label="this.$t('message.violation_state')">
          ...
          <b-form-group id="risk-type-form-group" :label="this.$t('message.risk_type')">
            <b-form-checkbox-group
          ...
          <b-form-group id="analysis-type-form-group" :label="this.$t('message.analysis_state')">
            <b-form-checkbox-group
          ...
          <b-form-group id="occurred-on-date-form-group" :label="this.$t('message.occurred_on')">
            <b-datepicker id="occurred-on-datepicker-from" v-model="occurredOnDateFrom" :max="occurredOnDateTo" :placeholder="this.$t('message.from')"></b-datepicker>
            <b-datepicker id="occurred-on-datepicker-to" v-model="occurredOnDateTo" :min="occurredOnDateFrom" :placeholder="this.$t('message.to')"></b-datepicker>
          </b-form-group>
          ...
          <b-form-group id="policy-form-group" :label="this.$t('message.policies')" v-permission="PERMISSIONS.POLICY_MANAGEMENT">
            <b-form-checkbox-group
...
<script>
  ...
  export default {
    ...
    computed: {
      watchers() {
        return [this.showInactive, this.showSuppressed, this.violationStateSelected, this.riskTypeSelected,
          this.policySelected, this.analysisStateSelected, this.occurredOnDateFrom, this.occurredOnDateTo]
      }
    },
    beforeMount() {
    ...
    methods: {
      initializePolicies: function () {
        ...
        if (hasPermission(permissions.POLICY_MANAGEMENT, this.decodedToken) || hasPermission(permissions.ACCESS_MANAGEMENT, this.decodedToken)) {
          this.axios.get(policyUrl).then((response) => {
            if (response.data) {
              response.data.forEach(policy => this.policyOptions.push({text: policy.name, value: policy.uuid}))
            }
          }).catch((error) => {
      ...
      apiUrl: function () {
        ...
        url += "?showInactive=" + (this.showInactive === "true") + "&suppressed=" + (this.showSuppressed === "true");
        if (this.violationStateSelected && this.violationStateSelected.length > 0) {
      ...
      onLoadSuccess: function () {
        loadUserPreferencesForBootstrapTable(this, "PolicyViolationAudit", this.$refs.table.columns);
      },
    },
    data() {
      return {
          ...
          },
          {
            ...
            formatter: (value, row, index) => {
              if (row.component) {
                ...
                let dependencyGraphUrl = xssFilters.uriInUnQuotedAttr("../../../projects/" + row.project.uuid + "/dependencyGraph/" + row.component.uuid)
        ...
        options: {
          ...
          pageSize: (localStorage && localStorage.getItem("PolicyViolationAuditPageSize") !== null) ? Number(localStorage.getItem("PolicyViolationAuditPageSize")) : 10,
          sortName: (localStorage && localStorage.getItem("PolicyViolationAuditSortName") !== null) ? localStorage.getItem("PolicyViolationAuditSortName") : 'timestamp',
          sortOrder: (localStorage && localStorage.getItem("PolicyViolationAuditSortOrder") !== null) ? localStorage.getItem("PolicyViolationAuditSortOrder") : 'desc',
          icons: {
            refresh: 'fa-refresh'
          },
          ...
          onPageChange: ((number, size) => {
            if (localStorage) {
              localStorage.setItem("PolicyViolationAuditPageSize", size.toString())
            }
          }),
          onColumnSwitch: ((field, checked) => {
            if (localStorage) {
              localStorage.setItem("PolicyViolationAuditShow"+common.capitalize(field), checked.toString())
            }
          }),
          onSort: ((name, order) => {
            if (localStorage) {
